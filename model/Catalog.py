...
class Catalog:
    ...
    def populate(self, books, magazines, movies, music):
        ...
        if movies is not None:
            for movie in movies:
                self.item_catalog.append(Movie(movie[0], movie[1], "mo", movie[2], movie[3], movie[4], movie[5], movie[6], movie[7], movie[8], movie[9], movie[10], movie[11]))

        if music is not None:
            for item in music:
                self.item_catalog.append(Music(item[0], item[1], "mu", item[2], item[3], item[4], item[5], item[6], item[7], item[8]))
